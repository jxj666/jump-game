if (typeof HdGame == "undefined") {
    ...
    b.showMsg = (function() {
        ...
        return function(n, l, k, o, m) {
            ...
            b.fadOut(e, function() {
                if (m && isJoinThemeGame && !b.hasShowHdMarketingBox) {
                    b.hasShowHdMarketingBox = true;
                    setTimeout(function() {
                        $(document.body).append('<div class="hdMarketingBox"><div class="weui-mask" style="z-index:5000"></div><div class="weui-mask" style="z-index:5000" onclick="$(\'.hdQrcodeImg\').hide(); $(\'.closeQrcodeBtn\').show()"></div><div class="closeQrcodeBtn" onclick="$(\'.hdMarketingBox\').remove()">关闭</div><img class="hdIconImg" src=' + _resRoot + '/image/hdIcon.png?v=201711290948/><div class="hdTextBox"><p class="hdTextBox_one">成聚移动互动</p><p class="hdTextBox_two">更专业的营销工具</p><div class="hdTextBox_three" onclick="$(\'.hdQrcodeImg\').show(); $(\'.closeQrcodeBtn\').hide();HdGame.logDog(1000219,10);HdGame.logDog(1000219, 11 + g_config.isAOpenId);" >免费创建营销活动</div></div><img class="hdQrcodeImg" src="' + _resRoot + "/image/hdQRcode/themeGame" + g_config.isAOpenId + '.png?v=201712061434" /></div>')
                    }, 1300);
                    b.logDog(1000219, 9)
                }
            });
    ...
    (function() {
        ...
        b.openAwardDetail = function(H) {
            ...
            if (_manage) {
                ...
                $("#awardCodeLayer,#awardDetailBox").find(".codeImg").attr("src", "//" + b.gameDomain + "/manage/qrCode.jsp?cmd=qrurl&siteUrl=" + b.encodeUrl(H.awardLevel == 900 ? ("id=" + H.id + "&code=" + H.code1 + "&gameId=" + w) : ("code=" + H.awardCode + "&gameId=" + w)));
        ...
        function k() {
            ...
            f.find("#log_share").on("touchstart", function() {
                ...
                if (g_config.test) {
                    ...
                    var o = "//" + b.gameDomainIgnoreStyle + "/hdtest.jsp?cmd=setUrl&url=" + p;
                    window.open(o)
                }
            });
    ...
    b.initJsFoot = function(e) {
        ...
        $(function() {
            ...
            var p = b.decodeHtml(e.hostLink).replace("//", "");
            var q = b.decodeHtml(e.menuLink).replace("//", "");
            ...
            if (!_manage) {
                ...
                }).addResizableFn("start", function(r, s) {
                    $(this).add($(this).parent()).css({
                        "max-height": (parseInt($("#resule-gift-scrollWrap").css("height")) - $(this).offset().top) / g_rem + "rem"
                    })
                }).addResizableFn("resize", function(s, t) {
                    $(this).add($(this).parent()).css({
                        left: 0,
                        top: 0,
                    });
                    var r = parseInt($("#resule-gift-scrollWrap").css("height")) - $(this).offset().top;
                    if ($(this).height() > r) {
                        $(this).add($(this).parent()).css({
                            "max-height": r / g_rem + "rem"
                        })
                    }
                });
                ...
                hg.on("save", function(r) {
                    var s = "";
                    $("#ruleBox .poupLine").each(function(u, v) {
                        var t = $(this).data("sortkey");
                        if (t) {
                            s += t
                        }
                    });
                ...
                (function() {
                    if (hg.sound.list && hg.sound.list.length > 0) {
                        var r = ".home,.gameBgBox";
                        if (g_config.style == 34) {
                            r = "#gameTopBar,#gameLayerBox1,#gameLayerBox2,#gameLayerBox3"
                        } else {
                            if (g_config.style == 45 || g_config.style == 60) {
                                r = ".gameInfoBoxMain"
                            } else {
                                if (g_config.style == 48) {
                                    r = "#gameContent,.friendEnterBoxer"
                                } else {
                                    if (g_config.style == 9) {
                                        r = "#openBox"
                                    } else {
                        ...
                        $$(function() {
                            parent.Edit.addEditBtn({
                                ...
                                filter: function(s) {
                                    return $(s).closest("#advertising").length == 0
                                }
                            })
                        })
                    }
                    $$(function() {
                        ...
                        b.saveModuleLayerImg(function() {
                            ...
                            if (hg.edit && hg.edit.bgList) {
                                $.each(hg.edit.bgList, function(u, v) {
                                    t[v.from] = v.defSrc
                                })
                            }
                            return t
                        ...
                        parent.Edit.addEditBtn({
                            ...
                            onclick: function() {
                                ...
                                if (hg.edit && hg.edit.bgList) {
                                    var w, v;
                                    for (var u = hg.edit.bgList.length - 1; u >= 0; u--) {
                                        w = hg.edit.bgList[u];
                                        v = $(w.from);
                                        if (v.length > 0 && v.eq(0).is(":visible")) {
                                            v.removeClass("hd-game-theBg-fillAuto");
                                            $.extend(x, w);
                                            t = true;
                                            break
                                        }
                                ...
                                hg.fire("editBackground", x, t);
                                parent.Edit.showEditByGame(x._flag, "", [x]);
                                return false
                            }
                        })
                    })
    ...
    b.getGameRule = (function() {
        ...
        return function(g, f) {
            ...
            (function() {
                // var j = `{"rt":0,"success":true,"data":{"info":"{'startTime':1514433981297,'flow':2103996046,'rule':{'xList':[0.6822,6.8319,4.4748,9.8691,3.2068,0.4265,1.6577,10.477,5.6111,0.4257,7.5249,2.7141,6.8543,7.4581,7.1842,0.9492,3.8997,0.2572,3.7545,9.3783,2.1677,8.6236,6.0274,2.9871,7.9391,2.3917,4.5719,9.0508,4.7037,5.1196,2.309,0.2231,6.1896,10.3411,4.4852,8.6056,4.9972,6.9089,3.5772,4.3612,5.4036,9.7322,7.4688,1.933,8.5031,7.5611,1.7482,2.2575,4.3939,1.8993,7.6908,5.514,5.6277,1.6312,4.4065,10.4371,2.0923,6.5364,1.7512,7.5292,7.406,8.525,5.9252,2.149,8.7663,9.1689,7.2061,6.4188,1.507,6.4233,0.4942,10.1907,0.3896,10.3289,1.7788,2.4334,0.8732,7.961,5.2707,7.853,0.2836,10.4446,4.7833,7.8433,2.9267,7.9853,1.4877,2.4654,2.2766,8.5779,10.4094,7.596,5.3971,8.838,8.4852,1.645,8.747,1.2711,10.1465,4.338],'cWidth':5.5,'jWidth':4.0}}","sign":"0bf708c0e3eb4ee3e51cca521b023fb8"},"msg":"操作成功"}`;
                // var h = $.parseJSON(j);
                var h={};
                if (true) {
                    // _ruleInfo.rule = h.data;
                    ...
                    //     h.data = _ruleInfo.rule.info.rule
                    // }
                    h.data = { startTime: 1514433981297, flow: 2103996046, rule: { xList: [0.6822, 6.8319, 4.4748, 9.8691, 3.2068, 0.4265, 1.6577, 10.477, 5.6111, 0.4257, 7.5249, 2.7141, 6.8543, 7.4581, 7.1842, 0.9492, 3.8997, 0.2572, 3.7545, 9.3783, 2.1677, 8.6236, 6.0274, 2.9871, 7.9391, 2.3917, 4.5719, 9.0508, 4.7037, 5.1196, 2.309, 0.2231, 6.1896, 10.3411, 4.4852, 8.6056, 4.9972, 6.9089, 3.5772, 4.3612, 5.4036, 9.7322, 7.4688, 1.933, 8.5031, 7.5611, 1.7482, 2.2575, 4.3939, 1.8993, 7.6908, 5.514, 5.6277, 1.6312, 4.4065, 10.4371, 2.0923, 6.5364, 1.7512, 7.5292, 7.406, 8.525, 5.9252, 2.149, 8.7663, 9.1689, 7.2061, 6.4188, 1.507, 6.4233, 0.4942, 10.1907, 0.3896, 10.3289, 1.7788, 2.4334, 0.8732, 7.961, 5.2707, 7.853, 0.2836, 10.4446, 4.7833, 7.8433, 2.9267, 7.9853, 1.4877, 2.4654, 2.2766, 8.5779, 10.4094, 7.596, 5.3971, 8.838, 8.4852, 1.645, 8.747, 1.2711, 10.1465, 4.338], cWidth: 5.5, jWidth: 4.0 } }
                    g && g(h)
                } else {
                    b.showMsg("系统错误，请刷新重试")
                }
